...
The Personal profile runs a loopback TCP relay for the Windows OpenPencil MCP endpoint. The relay listens on `127.0.0.1:7601` in WSL and forwards raw bytes to port `7601` on the WSL default IPv4 gateway. Windows then forwards that connection to OpenPencil at `127.0.0.1:7600`.
...
Windows must forward the current WSL gateway address on port `7601` to `127.0.0.1:7600`. A narrow Windows firewall rule must allow traffic from the current WSL address.

Keep native Windows `mise` configuration separate from the WSL MFZ configuration. The native Windows config belongs at `%USERPROFILE%\\.config\\mise\\config.toml`; do not point it at the WSL-rendered `~/.config/mise/conf.d` fragments. If Windows tools are launched from a WSL UNC working directory, set `ceiling_paths` in the native Windows `%USERPROFILE%\\.config\\mise\\miserc.toml` to the WSL home path so Windows `mise` does not discover WSL configuration above the working directory.
...
For the one-time port migration, run the following in an elevated Windows PowerShell using the current WSL gateway address. Add and verify the replacement before deleting the old mapping:

```powershell
netsh interface portproxy add v4tov4 listenaddress=172.19.128.1 listenport=7601 connectaddress=127.0.0.1 connectport=7600
netsh interface portproxy show v4tov4
netsh interface portproxy delete v4tov4 listenaddress=172.19.128.1 listenport=7600
```

Update the matching inbound firewall rule from local TCP port `7600` to `7601`, preserving its current WSL remote-address restriction. The port proxy and firewall rule are machine-local and remain outside MFZ.
...
ss -H -ltnp 'sport = :7601'
```

The listener must be `127.0.0.1:7601`. The service discovers the gateway once at startup and records the selected address in the journal. It logs lifecycle and gateway state only. It does not log payload bytes, headers, tokens, document names, or content.
...
```sh
curl --connect-timeout 1 --max-time 5 --include http://127.0.0.1:7601/health
```

With OpenPencil and the Windows forwarding path available, the response is `HTTP/1.1 200 OK` and reports the running OpenPencil version. Check `authRequired` without printing or saving a token.

Use the loopback URL for MCP clients:

```text
http://127.0.0.1:7601/mcp
```
...
If the service reports `EADDRINUSE`, another WSL process owns `127.0.0.1:7601`. Identify that process before taking any action. Do not stop an unknown owner.
...
curl --connect-timeout 1 --max-time 5 --include "http://${gateway}:7601/health"
```
