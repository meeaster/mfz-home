...
The coordinator uses each subagent's configured default model and variant. Orchestrator Mode chooses roles and continuity, not model overrides or automatic escalation. Reviews can measure costs at the actual recorded model's rates without turning those observations into dispatch-time model selection. A model's price or reputation alone does not establish whether a particular result is trustworthy.
...
For artifact-author, agent-author, and explicitly authorized super-worker, a latest recorded request-input context above 150,000 tokens requires a fresh session at the next dispatch. This includes small corrections and near-complete units. The coordinator transfers the artifact, accepted decisions, remaining work, relevant evidence, and safely preserved state. A missing material detail calls for a bounded handoff repair, not resuming past the limit. The rule does not interrupt an active call, select another role, or override super-worker's human-selection requirement. Unknown context stays unknown; cumulative processed input is not the current window.
