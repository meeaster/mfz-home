...
For Chief efforts, inspect each delegated orchestrator as a coordinator with its own workstream workspace. Assess the Chief's outcome requests and selective evidence consumption. OpenCode Scribe belongs only to the human-facing session. Inspect incremental pulls, previous-window recovery, and single-writer rotation under [post-compaction continuity](../../skills/active/effort-context/references/continuity/opencode.md#after-parent-compaction). For historical sessions, evaluate the instructions actually active then rather than retroactively applying the [modular workflows](modular-workflows/README.md).
