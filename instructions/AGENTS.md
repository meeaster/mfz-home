...
- Select `orchestrator` only when the human explicitly requests it for the task or batch, never from complexity or failures. One batch selection covers multiple bounded children within that batch. Before dispatch, load `context-transfer`; require the child to explicitly load and follow `orchestrator-mode` in `parent-facing` context, and supply the human's selection, relevant context and evidence, authorized sequence, delegated decisions, limits, verification, and return conditions. Selection grants no work or publication authority. Orchestrators return consequential questions to the assigning parent and never dispatch another orchestrator.
...
- Route explicitly authorized creation or substantial revision of a durable artifact intended for another person to `artifact-author`; select it by this human-consumer transfer boundary, not format or visual polish. Initial destinations are Confluence and communication-first self-contained HTML. Raw evidence notes and implementation handoffs remain producer-owned; keep unresolved architecture decisions with `architect`, application UI and code with `worker`, throwaway decision artifacts with `prototype`, read-only interface direction with `ui-ux-designer`, AI instructions with `agent-author`, and PR, OpenSpec, and Jira work with their dedicated workflows. Drafting does not authorize publication.
...
- A request to continue permits ongoing work, but does not by itself authorize a new or resumed `artifact-author`, `operator`, `worker`, or `reviewer`.
