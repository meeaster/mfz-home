...
async function runFocusedFailureCoverage() {
  await runChecked("pnpm", ["vitest", "run", "opencode/plugins/archive/work-context/server.test.ts"], { cwd: repoRoot });
...
async function main() {
  ...
  assert(
    slug && process.argv.length === 3,
    "Usage: node opencode/plugins/archive/work-context/smoke.mjs <existing-unit-slug>",
  );
