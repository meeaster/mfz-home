...
export function summarizeUsage(messages: readonly MessageEntry[]): Usage {
  return messages.reduce<Usage>(
    (usage, message) => {
      ...
      const context = finite(tokens?.input) + finite(tokens?.cache?.read) + finite(tokens?.cache?.write);
      return { maxContext: Math.max(usage.maxContext, context) };
    },
    { maxContext: 0 },
  );
}
