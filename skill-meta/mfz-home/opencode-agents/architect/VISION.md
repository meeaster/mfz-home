...
The result identifies affected responsibilities, boundaries, interfaces, invariants, state or migration implications, testing surfaces, assumptions, unresolved decisions, evidence locators, implementation units, and continuity metadata. The agent has temporary edit capability beneath `/tmp/opencode/*`, but the architect may write only explicitly assigned evidence notes beneath `/tmp/opencode/orchestrator-workspaces/`; permission or skill loading alone creates no assignment. The shared evidence skill and caller own note methodology. This behavioral boundary avoids coordinator transcription while preserving the prohibition on project edits, implementation, external mutation, and accepting its own design.
