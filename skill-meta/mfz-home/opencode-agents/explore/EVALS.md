...
- From a Location and project outside `/tmp/opencode/`, the configured edit exception matches any file beneath that temporary root. The orchestration workflow still assigns producer files only beneath `/tmp/opencode/orchestrator-workspaces/`; producer ownership remains a dispatch constraint, not a permission restriction. Paths outside `/tmp/opencode/` receive no allow from this exception.
...
- From a Location or project containing `/tmp/opencode/`, the caller or producer returns the placement blocker rather than changing permissions or bypassing the tool policy.
