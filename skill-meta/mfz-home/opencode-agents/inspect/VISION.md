...
The human chose default prompt inheritance because no explicit system-level file-writing prohibition was established for Inspect. Configured permissions allow temporary edits under `/tmp/opencode/*`; the caller explicitly assigns evidence notes only beneath `/tmp/opencode/orchestrator-workspaces/` and requires `orchestrator-task-evidence` for that narrower contract. Source and external state remain read-only under the assignment. Permission and skill loading alone authorize no file. `../explore/MAINTENANCE.md` owns permission evidence and supported-Location limits. The earlier reported live refusal remains unexplained; test the default-prompt path before adding system instructions. Keep the existing shell policy unchanged.
