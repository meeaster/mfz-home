...
A loaded workflow must explicitly route to this role. The caller supplies the exact proposed action, target and environment, selectors, expected affected resources, dependency and propagation paths, containment, reversibility and rollback facts, current-state evidence with locators and freshness, unresolved assumptions, and accepted authority. The agent treats that packet as evidence rather than proof and uses ordinary inspection capabilities to verify consequential claims through operations whose semantics are clearly read-only.
