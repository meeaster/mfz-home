...
These are verification commands, not a record of a successful run. Current scenario expectations and evidence limits are in [EVALS.md](EVALS.md). Consult [historical evidence](LOG.md) for the incomplete-report failure and older Bundle experiments, not for current command syntax.
