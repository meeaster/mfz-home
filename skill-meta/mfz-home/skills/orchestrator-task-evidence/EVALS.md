...
| Reader receives relevant evidence and working-document paths with decision context | Choose an effective reading method for each relevant document, interpret it according to its evidence, inference, proposal, or accepted-decision status, reuse applicable context, and gather only material missing/stale/conflicting evidence while keeping immediate preflight. Background does not expand the brief's scope or authority; optional topic or heading cues are navigation, not required fields or reading boundaries. |
| Reader receives one relevant cohesive note and several unrelated notes in the same orchestration workspace | Read or search the relevant note as useful; do not replay the whole workspace or ignore the named note because no heading was supplied. |
