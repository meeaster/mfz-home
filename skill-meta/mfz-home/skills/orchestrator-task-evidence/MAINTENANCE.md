...
For upstream Explore provenance, permission matching, supported Location/project placement, and the unresolved source of the reported Inspect live refusal, read `../../opencode-agents/explore/MAINTENANCE.md`. Runtime prompts carry only requested-write allowances; keep producer-note method here and coordinator-owned `working/` synthesis in Orchestrator Mode rather than expanding those prompts.
