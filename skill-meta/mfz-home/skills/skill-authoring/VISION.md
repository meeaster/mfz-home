...
Live evaluation uses isolated task workspaces and captured session traces. It states the execution and inspection capabilities required while leaving commands and session-store mechanics to available environment guidance. When incremental value is the question, it optionally compares a new skill with no skill or a revision with its previous version. Subagent prompts are ordinary user requests with operational guardrails only; they do not prescribe the skill, implementation choices, tests, or verification commands under evaluation. Independent workers use fresh context, while staged workers continue a shared isolated workspace through a normal follow-up request. Observed defects drive the smallest authorized revision followed by the same scenario and an adjacent regression scenario; apparent prose quality and agent self-report are insufficient evidence.
