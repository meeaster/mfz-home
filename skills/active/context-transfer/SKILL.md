...
skill's format, storage, review, and publication workflow. Do not replace its
artifact-specific semantics with a generic template.
...
the artifact's purpose requires it.

An explicit request to draft or revise authorizes only that draft or local edit.
Without separate explicit authority, do not commit, push, send, upload, publish,
or call an external write API. Follow the owning workflow's stronger approval
boundary when it has one.
