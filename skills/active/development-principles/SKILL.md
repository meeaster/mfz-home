...
description: Use when designing, implementing, testing, refactoring, coordinating, or reviewing software work. Guides happy-path-first design, evidence-based complexity, phase-aware scope, stable-boundary tests, and bounded code-health improvement.
disable-model-invocation: true
---

Development principles.

Apply these principles to the current software task, optimizing the design for the normal user flow. If the happy path is 95% of runtime behavior, it should be approximately 95% of the code readers see.
...
Apply the same principles differently across the development loop:

- during planning or design, identify the responsibilities, invariants, tests, and existing boundaries the proposed behavior will touch; for a substantial change, explicitly state whether those responsibilities stay in current files, move to a module, or change the folder layout, and compare the navigation and dependency cost; a no-change decision is valid but must be visible when structure is material; distinguish required feature work from optional maintenance so the human or coordinating agent can accept the scope
- during implementation, follow accepted design decisions and keep unplanned structural work bounded; do not turn a narrow feature into a separate reorganization unless the feature cannot remain clear or safe without it
- during verification or review, compare the implementation with the accepted design, including its boundaries, tests, persistence rules, and documentation; report missing accepted structure as implementation drift rather than silently redefining the design
- when coordinating work across changes, retain concrete code-health findings, reassess them after more evidence, and schedule focused maintenance only within the available authority
- when no separate design exists, implement the smallest clear change and surface larger structural opportunities for a human or coordinating agent to decide
...
- split a file when a cohesive responsibility has a stable interface, independent tests or callers, and a different reason to change from the surrounding code; prefer this evidence over line-count thresholds
- direct unit tests importing pure domain behavior through an executable or infrastructure entrypoint are strong evidence for a separate domain boundary; surface that evidence during design or review
- implement a file split when it is part of the accepted design or focused maintenance scope, or when it is necessary to keep the requested feature clear and safe; otherwise report it without expanding the feature
...
- optimize for fewer concepts, branches, and indirections, not fewer physical lines
- keep one logical action per line and expand control-flow bodies so readers can scan the sequence
- use the repository formatter when available; otherwise follow conventional human-readable formatting
- never compress multiple state changes or workflow steps onto one line merely to reduce line count
...
- before implementation, scan for material risks rather than cataloging every imaginable failure; estimate likelihood and consequence, and handle a risk upfront only when its cost justifies the added complexity
...
- prefer less semantic code, fewer names, fewer branches, and net-negative diffs when behavior permits; physical line compression is not simplification
- when changing an area, surface nearby code-health improvements that support long-term maintenance; implement them when they are accepted scope, necessary to keep the requested change clear or safe, or truly local behavior-preserving cleanup, and record larger work instead of expanding the feature opportunistically
...
A new file should own a real boundary, not merely shorten its former file. Direct unit tests importing pure domain behavior through an executable or infrastructure entrypoint are strong evidence that its interface and callers are already independent. Design and review should make that evidence visible; implementation should perform the split when the accepted scope includes it. A long file with one cohesive reason to change may still be the cheaper design.
...
Always assess test coverage as part of software work:

- for a new project, establish a minimal test runner and scripts from the beginning; lay foundations for both fast unit tests of domain behavior and integration tests through a real application boundary
- add the first representative unit and integration tests with the initial implementation so later agents can extend a known verification path instead of inventing one-off smoke commands
- keep unit and integration lanes independently runnable; the conventional default test command should run the fast unit lane, integration tests should require an explicit command, and a separate aggregate command may run the complete suite
- when test commands or their boundaries are not already clear, record when agents should run each lane in the repository's root agent guidance, such as `AGENTS.md`
- in an existing project with test infrastructure, extend its native patterns whenever behavior changes
- in an existing project without test infrastructure, weigh setup cost against the change's risk and expected future work; add a small native foundation when durable logic, parsing, persistence, migrations, state transitions, or important boundaries justify it, and otherwise state why focused non-persistent verification is proportionate
- every observed defect gets a regression test at the narrowest stable boundary that reproduces it

Tests should prove behavior through real boundaries. Use deterministic unit tests for domain rules and focused integration tests for wiring, persistence, protocols, and user-visible workflows. Do not test one-line helpers, duplicate implementation logic, or build large mock systems for small changes.

Keep the suite proportional:

- put combinatorial input variation and domain edge cases in fast unit tests
- use integration tests for a few representative happy paths, consequential failures, persistence contracts, migrations, and integration seams
- do not repeat the full unit-test matrix through a slower boundary merely to increase coverage
- measure or observe test duration as the suite grows; preserve a fast feedback lane and address a slow integration suite only from runtime evidence
- prefer the smallest real fixture and boundary that prove the behavior; avoid broad end-to-end environments when an owned integration boundary answers the same question
...
At the end of the current task, run the default fast lane, any affected integration lane through its explicit command, and focused verification. Run the aggregate suite when project policy or change risk warrants it. If no durable test was added or run, record the evidence-based reason. When an accepted design exists, check the implementation, tests, documentation, and structural boundaries against it and report unresolved drift. Remove only disposable artifacts created during this task when their disposal is explicitly within scope. Preserve existing work, design artifacts, fixtures, and user files. The result should feel boring, obvious, typed, cohesive, and native to the codebase.
