...
Use the current [OpenCode V2 plugin documentation](https://opencode.ai/v2/docs/build/plugins) as the normative server-plugin API and configuration source. Fetch it before selecting server entrypoints, hooks, config shape, or package metadata. The stable target is OpenCode 2.0.0 with `@opencode/plugin@2.0.0`; verify that package's published exports before changing a plugin. For native TUI behavior the server page does not document, inspect the matching release source and [runtime loading](references/runtime-loading.md) rather than extending the server contract by analogy.
...
3. Inspect the active OpenCode version and the plugin's actual runtime package boundary. Declare every non-host runtime import in that plugin's `dependencies`, make it resolvable from the rendered entrypoint, and match `@opencode/plugin@2.0.0` to the stable 2.0.0 CLI. Type-only imports are development dependencies, not rendered runtime requirements.
