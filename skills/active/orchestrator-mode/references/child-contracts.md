...
For large context transfers, use a pointer brief. Always distill the authoritative objective, accepted decisions, authority limits, verification, and stop conditions into the brief rather than asking the child to pull them. Put supporting detail behind an explicit `session_context` directive that names the target `sessionID`, says what to focus on, and tells the child to treat the transcript as evidence. Reuse one scribe session within an effort and pass it no marker; the scribe manages its own position across cycles. For a one-off non-scribe delegate, the coordinator may explicitly seed `sinceMarker` to bound the requested range to recent material. Require direct file reads for exact content because pulls bound tool-result prefixes. This is v1.1 guidance; revise it from observed usage rather than treating its present form as final.
