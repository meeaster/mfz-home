...
A fresh child dispatched by the parent-facing orchestrator needs the decision-relevant meaning its own context does not supply:
...
Scribe dispatches require pointer briefs. The brief carries the authoritative core—objective, scope, output paths, authority, and stop conditions—plus an explicit `session_context` directive naming the target `sessionID` and focus. Do not inline decided-state content that the pull supplies, except exact wording wanted verbatim or corrections to the source context. Reuse each coordinator's scribe session within its workspace scope and pass no marker; the scribe manages its own cursor across cycles. One-off non-scribe delegates may still receive an explicit `sinceMarker` for a separately authorized pull that does not retrieve Chief context. Require direct file reads for exact content because pulls bound tool-result prefixes.
...
Source facts belong in this cycle. Decisions, consultation, expanded access/scope, and mutation remain with their owning authority. At the depth limit, the parent-facing orchestrator owns additional gathering.
