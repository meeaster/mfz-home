...
A fresh child needs this correctness-critical brief core when its own context does not supply it:
...
Scribe dispatches use pointer briefs. Keep the pointer core to objective, owned output path, authority, stop conditions, and an explicit `session_context` directive naming the target `sessionID` and focus. Do not inline decided-state that the pull supplies, except exact wording wanted verbatim or corrections to the source context. All seven of seven observed target scribe attempts duplicated decided-state supplied by pulls or context. Prompt input is billed to the delegator, so a scribe prompt larger than its useful output defeats the pointer's purpose. The available comparison distinguishes prompt size from patch payload and final return text; it does not measure exact semantic file delta, so do not claim a stronger prompt/output result. Reuse each coordinator's scribe session within its workspace scope and pass no marker; the scribe manages its own cursor across cycles. One-off non-scribe delegates may still receive an explicit `sinceMarker` for a separately authorized pull that does not retrieve coordinator context. Require direct file reads for exact content because pulls bound tool-result prefixes.
...
Source facts belong in this cycle. Decisions, consultation, expanded access/scope, and mutation remain with their owning authority. At the depth limit, the dispatching coordinator owns additional gathering.
...
For possible design conflict, return the smallest needed architecture-evidence request through the mode's return surface. The coordinator classifies it and routes any gathering through the selected dispatcher, surfaces it with the pending choice, and explains the value of another architect turn. Resume a still-authorized engagement or obtain new authority. Requirement, access, scope, and authority changes go to the decision recipient. Ordinary implementation/environment failures that fit the design need no architect.
