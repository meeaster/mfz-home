...
| `artifact-author` | For diagrams and HTML explanation pages, resume small focused corrections when retained context remains useful; prefer a fresh session for substantial revisions, carrying the current artifact and compact accepted context. Keep immediate checks and fixes with the revision owner. Other artifacts may resume feedback, correction, or approved publication in the same editorial lifecycle. Start fresh for a distinct artifact/set or materially changed audience, destination, authority, or outcome. |
