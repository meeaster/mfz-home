...
| `scribe` | Coordinator-routed transcription of decided state into explicitly assigned orchestration workspace state, evidence cataloging, synthesis notes, or enablement artifacts. Not a general prose writer or an owner for work whose errors propagate into implementation. |
...
The coordinator decides; the scribe transcribes. Keep decisions and acceptance with the coordinator, and keep OpenSpec proposals in their propose and review workflows. Routine workspace upkeep may run as a background scribe dispatch; decision-gating artifacts run in the foreground so the coordinator can check them before dependent work. This is v1 routing guidance to revise after observed scribe usage.
