...
**Mindframe-Z local overlay:** A standalone store is the planning root, not an implementation workspace. The selected store is the only planning root this workflow may archive; never infer source edit roots or linked-repository ownership from the selected store, its `allowedEditRoots`, or a workset. If status reports workspace planning, stop rather than moving workspace changes into a repo-local archive or editing linked repositories.
