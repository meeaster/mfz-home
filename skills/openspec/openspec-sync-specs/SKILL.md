...
**Mindframe-Z local overlay:** A standalone store is the planning root, not an implementation workspace. Use `planningHome.root` for main specs and never infer source edit roots or linked-repository ownership from the selected store, its `allowedEditRoots`, or a workset. If status reports workspace planning, stop rather than falling back to repo-local paths or editing linked repositories.
