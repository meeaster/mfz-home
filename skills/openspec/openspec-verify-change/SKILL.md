...
**Mindframe-Z local overlay:** A standalone store supplies planning artifacts; implementation verification must inspect an explicitly selected implementation workspace. Never infer source edit roots or ownership from the selected store, `planningHome`, `allowedEditRoots`, or a workset. If the implementation workspace is not explicit, ask the user to select it before inspecting source.
